...
3. 데이터 생성 기준일을 확인합니다. 아이디 입력은 필요 없습니다. 앱은 소유 계정이나 마지막 분할 파일 누락을 자동으로 확인할 수 없으므로 같은 계정의 모든 파일인지 확인해야 합니다.
4. 현재 맞팔/일방 팔로우를 확인하고 전체 결과를 검색·페이지 이동·CSV 다운로드합니다.
5. **기록 저장을 선택한 경우에만** 브라우저 IndexedDB에 저장합니다. 저장할 기존 기록을 직접 선택하면 같은 묶음의 이전 기준일과 비교합니다. 기본값인 새 기록은 다른 기록과 자동 연결하지 않습니다.
...
Next.js 16 / React 19 / TypeScript / Tailwind CSS 3 / Zustand / IndexedDB를 사용합니다. Web Worker 빌드와 검증 환경을 일치시키기 위해 Webpack을 명시합니다. 차트는 통계 화면에서만 지연 로드합니다. 글꼴은 자체 호스팅하며, Vercel Web Analytics에는 개인 데이터 없는 사용 통계만 보냅니다.
...
- 아이디 입력 없이 분석합니다. 저장 시 기본값은 새 기록 묶음이며, 같은 계정의 파일일 때 기존 기록을 선택해 이어 저장하고 비교할 수 있습니다. 묶음별 최대 10개를 보관하며 같은 묶음·같은 기준일은 교체합니다. 서로 다른 계정의 기록은 연결하지 마세요. 날짜는 사용자가 제공하므로 원본 생성일을 정확히 입력해야 합니다.
...
- Vercel Web Analytics에는 방문 페이지와 `analysis_completed`, `download_guide_viewed`, `share_button_clicked`, `install_prompt_accepted`, `home_screen_launch`만 전송합니다. 이벤트 속성은 보내지 않으며 URL의 쿼리와 해시를 제거합니다. 인스타그램 아이디·파일명·관계 목록·분석 수치는 전송하지 않습니다.
