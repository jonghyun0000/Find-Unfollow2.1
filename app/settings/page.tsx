        ...
        <section className="glass rounded-2xl p-5">
          ...
          {confirming ? (
            <div className="mt-4 space-y-3">
              <p className="text-sm text-rose-200">삭제한 기록은 복구할 수 없습니다.</p>
              <div className="flex gap-3">
                <button
                  disabled={pending}
                  className="rounded-xl bg-rose-500 px-4 py-3 font-semibold disabled:opacity-50"
                  onClick={async () => {
                    setPending(true);
                    await resetAll();
                    setMessage(
            ...
            <button
              className="mt-4 rounded-xl border border-rose-400/40 px-4 py-3 text-sm text-rose-200"
              onClick={() => {
                setConfirming(true);
                setMessage('');
              }}
