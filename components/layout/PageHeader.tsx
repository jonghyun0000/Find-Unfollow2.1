    ...
    >
      <div className="mx-auto max-w-md px-5 py-4 flex items-center gap-3">
        {back && (
          <button
            onClick={() => router.back()}
            className="-ml-2 grid place-items-center w-10 h-10 rounded-full hover:bg-white/5 active:scale-95 transition"
            aria-label="뒤로 가기"
          >
            <ChevronLeft size={22} />
          </button>
        ...
        <div className="flex-1 min-w-0">
          ...
          {subtitle && <p className="text-[13px] text-white/55 truncate">{subtitle}</p>}
        </div>
        {right}
      </div>
    </header>
